...
- `RepoExplorer/App/AppDependencies.swift` — composition root; `.live` builds the real client + UserDefaults history store and `makeSearchViewModel()`. `current()` swaps in canned data on an isolated UserDefaults suite when launched with `-uiTestStubResults` (pre-seeds recent searches with `-uiTestSeedHistory`; pre-fills the search with `-uiTestQuery <text>`); used by `RepoExplorerUITests`.
