...
RepoExplorer is a SwiftUI app for discovering GitHub repositories, built in phases as an MVVM app under Swift 6 strict concurrency. **Phases 1–3 (search + results list, metadata-only repo detail, persistent recent searches) are implemented.** Planned next: Phase 4 (pagination/polish). No third-party dependencies — Foundation + SwiftUI + URLSession only. GitHub search is unauthenticated (the client has a token-injection seam for later).

There is **no test target** — the project was deliberately slimmed to focus on functionality only. Correctness is exercised through SwiftUI previews (see `PreviewSupport.swift`) rather than XCTest/UI tests.
...
- `RepoExplorer/App/AppDependencies.swift` — composition root; `.live` builds the real client + UserDefaults history store, and `makeSearchViewModel()` wires them into the VM. Injecting a different `apiClient`/`historyStore` (e.g. the `PreviewGitHubAPIClient`) is how previews drive canned data.
...
- `RepoExplorer/Persistence/` — `SearchHistoryStore` (protocol, `Sendable`/`nonisolated` reqs) + `UserDefaultsSearchHistoryStore` (persisting `actor`) and `InMemorySearchHistoryStore` (the VM's default, used by previews). Case-insensitive dedup + MRU + cap shared via the `recentSearches(byInserting:…)` helper. Stores only `{query, date}` — API results are intentionally **not** cached; tapping a recent search re-fetches live.
...
- `RepoExplorer/Views/` — `SearchView` (root; `.searchable` + `.task(id:)`; `NavigationStack` + `.navigationDestination(for: Repository.self)`; recent-searches list in the idle state), `RepositoryRow` (card), `RepositoryDetailView` (metadata-only; gradient header + stat pills), `ErrorStateView`, `Components/AvatarView`, `Components/FlowLayout` (wrapping chips), `Components/SkeletonCard` (+ `LoadingListView` skeleton).
- `RepoExplorer/DesignSystem/Theme.swift` — design tokens: indigo/violet `accent`/`accentGradient`, semantic stat colors, `Spacing`, the `.repoCard()` surface modifier, `PressableButtonStyle`, `.shimmering()`. Global accent also set via the `AccentColor` asset + root `.tint(Theme.accent)`.
- `RepoExplorer/Extensions/` — small shared helpers (e.g. `Int.formattedCompact`).
- `RepoExplorer/PreviewSupport.swift` — `#if DEBUG` sample data + factories (`.make()`, `.samples`) and `PreviewGitHubAPIClient`, consumed by the SwiftUI `#Preview` blocks in the view files.
